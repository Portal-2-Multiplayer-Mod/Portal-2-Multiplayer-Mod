...
# This is the default masterDataFile.cfg structure
...
presetCatagories = {"officalp2mmmaps", "verifiedp2mmmaps", "unverifiedp2mmmaps"}
mapValues = {"verificationStatus", "mapFileName", "mapSupportFile", "data"}
dataSystemState: bool = True
# demDataFilesPath: str = GVars.configData["portal2path"]["value"] + GVars.nf + "SAVES"
...
def masterDataFileStructureCheck(masterDataFilePath) -> None:
    ...
    for keys in dataFileKeys:
        if keys not in presetCatagories:
...
def createNewMasterData() -> None:
    currentMasterData = (GVars.configPath + "\masterDataFile.cfg")
    try:
        os.remove(currentMasterData)
    except Exception as e:
        Log(f"The masterDataFile file was already removed or some other error occured: {str(e)}")

    newMasterDataFile = open(GVars.configPath + GVars.nf + "masterDataFile.cfg", "w+", encoding="utf-8")
    json.dump(defaultDataStructure, newMasterDataFile, indent=4)

    if os.path.exists(GVars.modPath + "\masterDataFile.cfg"):
        Log("DS: New masterDataFile.cfg created in p2mm directory...")
    else:
...
def dataSystemInitialization(refresh: bool) -> None:
    ...
    try:
        ...
        Log("DS: Checking our masterDataFile.cfg file...")
        if (not os.path.exists(GVars.configPath + GVars.nf + "masterDataFile.cfg")) or (os.path.getsize(GVars.configPath + GVars.nf + "masterDataFile.cfg") == 0):
            Log("DS: No masterDataFile.cfg detected, creating a new one...")
            createNewMasterData()
        else:
            Log("DS: masterDataFile.cfg retrieved!")
            masterDataFilePath = (GVars.configPath + GVars.nf + "masterDataFile.cfg")
    ...
    # Now we clean up the system and create the files needed for the datasystem-main.nut file to read when its called on later when a map loads
        ...
        try:
            if (dataSystemState == True):
                with open(GVars.dataSystemPath + "\datasystem-enabled.nut", "x") as dataSystemInitSuccess:
                    dataSystemInitSuccess.write('printl("The data system started successfully!")')
                    dataSystemInitSuccess.close()
            else:
                with open(GVars.dataSystemPath + "\datasystem-disabled.nut", "x") as dataSystemInitFail:
                    dataSystemInitFail.write('printl("The data system did not start successfully...")')
                    dataSystemInitFail.close()
                Log("DS: The data system encountered an error and it can't be enabled!")
