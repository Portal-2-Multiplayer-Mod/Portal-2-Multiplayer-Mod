...
def init() -> None:
    ...
    if (saveSystemInitContinue):
        if not os.path.exists(GVars.saveSystemNutPath + "/savesystem-main.nut"):
            ...
            Log("Launcher will continue but the save system will not be avaliable for the next play session...")
            Log("")
            saveSystemInitContinue = False
        else:
            saveSystemInitContinue = True
    ...
    if (saveSystemInitContinue):
        ...
        Log("Because of errors the save system will not be avaliable for the next playsession...")
        return False
