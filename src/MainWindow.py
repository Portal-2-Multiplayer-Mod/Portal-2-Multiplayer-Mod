...
class Gui:
    ...
    ###############################################################################
        ...
        self.CurrentMenuButtons: list[self.ButtonTemplate] = self.MainMenuButtons
...
#! END OF BUTTON FUNCTIONS
    ...
    def Update(self) -> None:
        ...
        for button in self.CurrentMenuButtons:
            button : self.ButtonTemplate = button
            indx += 1
        ...
        if (not self.LookingForInput):
            if GVars.iosd:
                keysIcon = pygame.image.load("GUI/assets/images/buttons.png").convert_alpha()
            else:
                keysIcon = pygame.image.load("GUI/assets/images/keys.png").convert_alpha()

            keysIcon = pygame.transform.scale(keysIcon, (W / 10, W / 10))
            self.screen.blit(
                keysIcon, ((W / 1.03) - keysIcon.get_width(), H / 1.22))
...
def DEVMOUNT() -> None:
    try:
        ...
        Log("Error deleting the old mod files, probably it didn't exist")
...
def MountModOnly() -> bool:
    ...
    #     DEVMOUNT()
    #     Ui.Error(translations["devmode_copied_from_local_repo"], 5, (75, 255, 75))
