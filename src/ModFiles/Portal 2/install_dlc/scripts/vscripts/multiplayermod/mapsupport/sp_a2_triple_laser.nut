...
if (!FutBolGamemode) {
    ...
    function FinishingSpawnFutbolInMap() {
            EntFire("p232servercommand", "command", "script CurrentlySpawningFutBol = false", 0.1)
            if (CanSpawnFutBol) {
    ...
    function SpawnFutbolInMap() {
        if (CanSpawnFutBol && !CurrentlySpawningFutBol) {
            ...
            EntFire("p232servercommand", "command", "script FinishingSpawnFutbolInMap()", 3)
        }
    }
    ...
    function MapSupport(MSInstantRun, MSLoop, MSPostPlayerSpawn, MSPostMapSpawn, MSOnPlayerJoin, MSOnDeath, MSOnRespawn) {
        ...
        if (MSPostPlayerSpawn==true) {
            EntFire("p232servercommand", "command", "say Game Starts In 30 Seconds", 0)
            EntFire("p232servercommand", "command", "say Game Starts In 20 Seconds", 10)
            EntFire("p232servercommand", "command", "say Game Starts In 10 Seconds", 20)
            EntFire("p232servercommand", "command", "say Game Starts In 5 Seconds", 25)
            EntFire("p232servercommand", "command", "say Game Starts In 4  Seconds", 26)
            EntFire("p232servercommand", "command", "say Game Starts In 3  Seconds", 27)
            EntFire("p232servercommand", "command", "say Game Starts In 2  Seconds", 28)
            EntFire("p232servercommand", "command", "say Game Starts In 1  Seconds", 29)
            EntFire("p232servercommand", "command", "say Game Starting!", 30)
            EntFire("p232servercommand", "command", "script StartGame()", 30)
            ...
            if (HasSpawned) {
                ...
                if (GameRunning) {
                    if (FutBolTeams[0].score  >= desiredscore) {
                        ...
                        DecEntFireByHandle(BlueGoalCounter, "Display", "", 0.1)
                        DecEntFireByHandle(RedGoalCounter, "Display", "",0.1)
                        EntFire("p232servercommand", "command", "say Game Restarts In 5 Seconds", 0)
                        EntFire("p232servercommand", "command", "say Game Restarts In 4  Seconds", 1)
                        EntFire("p232servercommand", "command", "say Game Restarts In 3  Seconds", 2)
                        EntFire("p232servercommand", "command", "say Game Restarts In 2  Seconds", 3)
                        EntFire("p232servercommand", "command", "say Game Restarts In 1  Second", 4)
                        EntFire("p232servercommand", "command", "portal2mprslv", 5)
                    }
                }
                ...
                if (GameRunning) {
                    ...
                    if (disp1&& disp2&&futent&&strip(futent.GetName()) == "futbol_in_map") {
                        if (CheckFutBolGoal(0, disp1, futent)) {
                            ...
                            EntFire("p232servercommand", "command", "script futent.SetOrigin(GetGoalPoint(disp1))", 0.1)
                            EntFire("p232servercommand", "command", "script CoreExplosion(futent.GetOrigin())", 0.9)
                        ...
                        if (CheckFutBolGoal(1, disp2, futent)) {
                                futent.__KeyValueFromString("targetname", "futbol_in_map_done")
                                DecEntFireByHandle(futent, "disablemotion", "", 0.05)
                                EntFire("p232servercommand", "command", "script futent.SetOrigin(GetGoalPoint(disp2))", 0.1)
                            EntFire("p232servercommand", "command", "script CoreExplosion(futent.GetOrigin())", 0.9)
                                DecEntFireByHandle(futent, "break", "", 1)
