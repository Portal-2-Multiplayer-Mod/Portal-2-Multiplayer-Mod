...
function SpawnFutBol(targetname, position) {
    SendToConsole("sv_cheats 1; ent_create prop_glass_futbol; sv_cheats 0")
    SendToConsole("script FinishFutBolSpawn(\"" + targetname + " \", Vector(" + position.x + ", " + position.y + ", " + position.z + "))")
}
